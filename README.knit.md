...
<img src="./logo_snakecase.png" height="175">
...
Since `preprocess` and `protect` allow to use regular expressions, `to_any_case()` becomes
very flexible and can achieve complex operations. Lets assume, that you want to translate
a string, which contains dots and decimal numbers, into snakecase. You want that the dots 
are treated as `"_"` in the output, but not if they are the separator of a decimal.

You can achieve this, while passing a regex (in this case a lookaround) to the `preprocess` argument,
which only translates those dots into `"_"`, that don't have a digit in front. The resulting
underscores between the digits can be cleaned via `protect = "\\d"`:
