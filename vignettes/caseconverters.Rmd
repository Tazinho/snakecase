...
to_any_case(strings2, case = "snake", preprocess = "\\s|-|\\:")
```

You can also specify a different separator than `"_"` or `""` via `postprocess`:

```{r, collapse = TRUE}
to_any_case(strings2, case = "snake", preprocess = "\\s|-|\\:", postprocess = "+++")

to_any_case(strings2, case = "big_camel", preprocess = "\\s|-|\\:", postprocess = "//")
```
